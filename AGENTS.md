...
### 連按 4 下重啟（快捷鍵彩蛋）
快捷鍵 1 或 2 連按 4 下（每下間隔 ≤200ms）觸發程式自動重啟。偵測在 `AppController._on_hotkey_toggle` 攔截層，per-hotkey 分開追蹤時間戳（`_rapid_press_times`）。前 3 下正常 toggle 錄音，不加延遲；第 4 下命中模式時 emit `restart_requested`，由 `app.py` 的 closure handler 執行「釋放 SingleInstance port → spawn 新進程 → quit 舊進程」。UI 按鈕與敲麥直連 `toggle_recording`，不經攔截，不會誤觸重啟。
相關：`AppController._on_hotkey_toggle`、`app.py` 的 `_handle_restart`。

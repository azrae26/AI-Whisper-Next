...
- 一般從原始碼執行 / `restart` Skill 重啟：主程式 log 為 `logs/ai_whisper_yyyyMMdd_HHmmss.log`（`paths.log_dir()`，專案根目錄下的 `logs/`）。
- 打包後 exe 執行：`logs/ai_whisper_yyyyMMdd_HHmmss.log`，路徑在 exe 同層底下的 `logs/`（例如 `dist/AI Whisper/logs/`）。
